...
**Название:** Допуск и листинг ценных бумаг — глобальное исследование

**Описание:** Многоуровневый пайплайн Deep Research для сбора и структурирования данных о требованиях к листингу и допуску к торгам ценных бумаг по 47 юрисдикциям и ~65 биржевым площадкам. Пайплайн включает 4 уровня сбора данных (юрисдикция → площадка → ячейка → проблемно-ориентированный анализ) с LLM-постобработкой на каждом уровне и пакетной классификацией по словарю параметров (П01–П23). Результат сохраняется в JSON-файлах и загружается в PostgreSQL.

**Цель:** Создать структурированную сравнительную базу данных регуляторных требований к листингу ценных бумаг в глобальном разрезе — как фундамент для анализа регуляторных паттернов, выявления связей параметров с регуляторными целями и поддержки работы по оптимизации российского регулирования.
...
00_docs/architecture/decision_NNN_*.md # Релевантные ADR (указаны в task_brief)
01_tasks/NNN_*/                        # Папка задачи
├── task_brief_01.md                   # Постановка (читает)
