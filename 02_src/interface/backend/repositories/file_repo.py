"""
FileDataRepository — reads pipeline output files from disk and returns typed API models.

Data layout (relative to COUNTRIES_DIR/{jurisdiction_ru}/):
  level_1/jurisdiction_card.json
  level_2/{venue_key}/venue_card.json
  level_2/{venue_key}/cells_list.json        # list or {"cells": [...]}
  level_3/{venue_key}/{cell_id}/3A_raw.json
  level_3/{venue_key}/{cell_id}/3A_validation.json
  level_3/{venue_key}/{cell_id}/3B_raw.json
  level_3/{venue_key}/{cell_id}/3B_validation.json
  level_3/{venue_key}/{cell_id}/3C_raw.json
  level_3/{venue_key}/{cell_id}/3C_validation.json
  level_3/{venue_key}/{cell_id}/pass2_ru.json   (preferred)
  level_3/{venue_key}/{cell_id}/pass2.json
  level_4/level4.json
  level_4/level4_validation.json
"""
...
# ---------------------------------------------------------------------------
# Low-level I/O helpers (no dependency on pipeline.storage)
# ---------------------------------------------------------------------------

def _load_json(path: Path) -> Any:
    """Load JSON from path. Returns None if file does not exist or parse fails."""
    if not path.exists():
...
def _load_cells_list(name_ru: str, venue_key: str) -> list[dict]:
    """Load cells list for a venue, handling both plain list and envelope formats."""
    path = COUNTRIES_DIR / name_ru / "level_2" / venue_key / "cells_list.json"
...
def _l3_base(name_ru: str, venue_key: str) -> Path:
    return COUNTRIES_DIR / name_ru / "level_3" / venue_key


def _load_l3_raw(name_ru: str, venue_key: str, cell_id: str, query_type: str) -> dict | None:
    """
    Load a 3A/3B/3C raw JSON for a given cell.
    Only looks in the cell_id-named folder (the canonical location).
    """
    path = _l3_base(name_ru, venue_key) / cell_id / f"{query_type}_raw.json"
    data = _load_json(path)
    if data is not None:
        logger.debug("Loaded %s for cell %s from %s", query_type, cell_id, path)
    return data


def _load_l3_validation(name_ru: str, venue_key: str, cell_id: str, query_type: str) -> dict | None:
    path = _l3_base(name_ru, venue_key) / cell_id / f"{query_type}_validation.json"
    return _load_json(path)


def _load_pass2(name_ru: str, venue_key: str, cell_id: str) -> dict | None:
    """Load pass2 data; prefers pass2_ru.json, falls back to pass2.json."""
    base = _l3_base(name_ru, venue_key) / cell_id
    for filename in ("pass2_ru.json", "pass2.json"):
        ...
        if data is not None:
            ...
            return data
    logger.warning("No pass2 file found for cell %s in %s/%s", cell_id, name_ru, venue_key)
    return None


def _cell_validation_status(name_ru: str, venue_key: str, cell_id: str) -> str:
    """
    ...
    statuses: list[str] = []
    for qt in ("3A", "3B", "3C"):
        val = _load_l3_validation(name_ru, venue_key, cell_id, qt)
        if val:
...
def _venue_type_label(raw_type: str | None) -> str:
    if raw_type is None:
        return raw_type or ""
...
def _lifecycle_label(key: str) -> str:
    return LIFECYCLE_PHASE_LABELS.get(key, key)


def _param_status_label(status: str) -> str:
...
# ---------------------------------------------------------------------------
# Helper: list all jurisdictions available on disk
# ---------------------------------------------------------------------------

def _list_jurisdiction_dirs() -> list[Path]:
    if not COUNTRIES_DIR.exists():
        logger.warning("COUNTRIES_DIR does not exist: %s", COUNTRIES_DIR)
        return []
    return [p for p in COUNTRIES_DIR.iterdir() if p.is_dir()]


def _list_venue_keys(name_ru: str) -> list[str]:
    level2_dir = COUNTRIES_DIR / name_ru / "level_2"
    if not level2_dir.exists():
        return []
    return [p.name for p in level2_dir.iterdir() if p.is_dir()]
...
class FileDataRepository:
    """
    Reads listing-requirements data from the pipeline output directory tree
    ...
    def get_jurisdictions(self) -> list[JurisdictionSummary]:
        ...
        for jdir in _list_jurisdiction_dirs():
            name_ru = jdir.name
            card = _load_json(jdir / "level_1" / "jurisdiction_card.json")
    ...
    def get_jurisdiction(self, name_ru: str) -> JurisdictionCard | None:
        ...
        l4 = _load_json(jdir / "level_4" / "level4.json")
        if l4:
            l4_val = _load_json(jdir / "level_4" / "level4_validation.json")
            val_status = "unknown"
            if l4_val:
    ...
    def get_venue(self, venue_key: str) -> VenueCard | None:
        ...
        vc = _load_json(COUNTRIES_DIR / name_ru / "level_2" / venue_key / "venue_card.json")
        if vc is None:
            logger.warning("venue_card.json not found for %s/%s", name_ru, venue_key)
            return None
        ...
        for cell in cells:
            ...
            has_3a = (_l3_base(name_ru, venue_key) / cid / "3A_raw.json").exists()
            has_3b = (_l3_base(name_ru, venue_key) / cid / "3B_raw.json").exists()
            has_3c = (_l3_base(name_ru, venue_key) / cid / "3C_raw.json").exists()

            pass2_base = _l3_base(name_ru, venue_key) / cid
            has_params = (pass2_base / "pass2_ru.json").exists() or (
                pass2_base / "pass2.json"
            ).exists()
        ...
        return VenueCard(
            ...
            venue_type=_venue_type_label(raw_type),
    ...
    def _find_jurisdiction_for_venue(self, venue_key: str) -> str | None:
        """Scan all jurisdiction folders to find which one contains this venue_key."""
        for jdir in _list_jurisdiction_dirs():
            level2 = jdir / "level_2" / venue_key
            if level2.exists():
                return jdir.name
        return None
    ...
    def get_cell_matrix(
        ...
        if cell_meta is None:
            logger.warning("cell_id=%s not found in cells_list for %s/%s", cell_id, name_ru, venue_key)
            # Try to continue with limited metadata
            cell_meta = {}
        ...
        # If none of the raw files exist, the cell effectively has no data
        ...
        # Text volumes for colour intensity in UI
        ...
        def col(col_index: int, col_key: str, status: MatrixCellStatus, volume: int) -> MatrixColumn:
            return MatrixColumn(
                col_index=col_index,
                col_key=col_key,
    ...
    def get_cell_parameters(
        ...
        data = _load_pass2(name_ru, venue_key, cell_id)
        if data is None:
            return None
        ...
        for raw_p in data.get("parameter_values", []):
            ...
            params.append(
                ParameterValue(
                    ...
                    lifecycle_phase_label=_lifecycle_label(phase_key),
                    value=raw_p.get("value", ""),
                    calculation_methodology=raw_p.get("calculation_methodology") or None,
    ...
    def get_all_parameters(self) -> list[ParameterSummary]:
        ...
        for jdir in _list_jurisdiction_dirs():
            ...
            for venue_key in _list_venue_keys(name_ru):
                ...
                for cell in cells:
                    cid = cell.get("cell_id", "")
                    data = _load_pass2(name_ru, venue_key, cid)
                    if data is None:
                        continue
                    for raw_p in data.get("parameter_values", []):
    ...
    def get_parameter_comparison(self, parameter_id: str) -> ParameterComparison | None:
        """
        ...
        for jdir in _list_jurisdiction_dirs():
            name_ru = jdir.name
            for venue_key in _list_venue_keys(name_ru):
                # Load venue card for display name
                vc = _load_json(
                    COUNTRIES_DIR / name_ru / "level_2" / venue_key / "venue_card.json"
                )
                ...
                for cell in cells:
                    ...
                    for raw_p in data.get("parameter_values", []):
                        ...
                        # Capture parameter name from first found occurrence
    ...
    def get_instrument_comparison(
        ...
        for jdir in _list_jurisdiction_dirs():
            name_ru = jdir.name
            jcard = _load_json(jdir / "level_1" / "jurisdiction_card.json")
            legal_family = jcard.get("legal_family") if jcard else None

            for venue_key in _list_venue_keys(name_ru):
                vcard = _load_json(
                    jdir / "level_2" / venue_key / "venue_card.json"
                )
                if not vcard:
                    continue
                ...
                for cell in cells:
                    ...
                    validation_status = _cell_validation_status(
